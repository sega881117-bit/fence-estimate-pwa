...
type NumericKey = 'length' | 'fencePrice' | 'targetTotal' | 'swingPrice' | 'slidingPrice' | 'swingCount' | 'slidingCount' | 'wicketPrice' | 'wicketCount' | 'deliveryPrice' | 'extension' | 'paint' | 'gravel' | 'pTrim';
type State = {
  ...
  deliveryPrice: number; extension: number; paint: number; gravel: number; pTrim: number;
};
...
const initial: State = {
  ...
  wicket: 'adjacent', wicketPrice: 0, wicketCount: 1, deliveryPrice: 0, extension: 0, paint: 0, gravel: 0, pTrim: 0,
};
const numericRules: Record<NumericKey, { min: number; integer?: boolean; blankWhenZero?: boolean }> = {
  ...
  deliveryPrice: { min: 0, blankWhenZero: true }, extension: { min: 0 }, paint: { min: 0 }, gravel: { min: 0 }, pTrim: { min: 0 },
};
...
const fixedTotalFor = (s: State) => {
  ...
  total += delivery;
  total += s.extension * 300 + s.paint * 250 + s.gravel * 300 + s.pTrim * 250;
  return total;
};
const fenceSpec = (material: Material, height: string) => {
  ...
  if (material === 'mesh3d') {
    const meshPost = height === '1.7' ? '2,9' : post;
    return { title: 'Забор 3D, высота ' + h + ' м.', details: ['секции 3D RAL 8017, толщина прутка 4 мм;', '3 скобы на 1 секцию;', 'столбы 60×60, L-' + meshPost + ' м, толщина стенки 2 мм, покраска DALI RAL 8017;', 'забивание столбов с шагом 2,5 м и заглублением 1,2 м.'] };
  }
...
export default function Home() {
  ...
  const quote = useMemo(() => {
    ...
    if (s.mode === 'standard') {
      ...
      if (s.paint) list.push({ title: 'Покраска каркаса', details: [], unit: 'м.п.', quantity: s.paint, unitPrice: 250, amount: s.paint * 250, requiresReview: true });
      ...
      if (s.pTrim) list.push({ title: 'Декоративная П-планка', details: [], unit: 'м.п.', quantity: s.pTrim, unitPrice: 250, amount: s.pTrim * 250 });
    }
  ...
  return <main className="app"><header className="head"><span className="mark">⌁</span><div><small>Локальный расчёт</small><h1>Смета забора</h1></div></header><section className="hero"><span>Предварительная смета</span><b>{money(quote.total)}</b></section><form onSubmit={submit}>
    ...
    </section><section className="card"><small>03 · Дополнительно</small><div className="grid">{numericInput('Удлинение столбов, м', 'extension')}{numericInput('Покраска каркаса, м', 'paint')}{numericInput('Забутовка щебнем на всю глубину, м', 'gravel')}{numericInput('Декоративная П-планка, м', 'pTrim')}</div>{price('Своя стоимость доставки, ₽', 'deliveryPrice', 'По метражу')}</section></>}
